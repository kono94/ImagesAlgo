...
/*
 * Info: Bildgr��e k�nnen Sie in der MyImage-Klasse �ndern durch die Konstanten:
 * 		 IMG_WIDTH
 * 		 IMG_HEIGHT
 * 
 * Vorgehen:
 * Map erstellen: diff color -> quantity
 * Diese Map in zwei array �bertragen: diff colors in m_colors speichern
 * 									   quantity in m_quantities speichern
 * Jetzt m_quantities mit quicksort sortieren und jeden swap parallel in m_colors ausf�hren
 * Grenze angeben, wieviele Farben erhalten bleiben sollen und diese in drei array (R, G, B) speichern
 * R nach Rot sortieren (gleiche Rotanteile dann nach Gr�n und dann noch nach Blau)
 * G nach Gr�n sortieren -II-
 ...
 * f�r jede Farbe rechts der Grenze, die n�heste Farbe erreichnen, indem:
 * bin�re suche aller Farbanteile der alten Farbe in den jeweiligen R-,G-,B- arrays ->startIndexR,G,B
 * Abstand berechnen
 * in jedem Array nach links und rechts laufen, bis der Abstand zum Farbanteil gr��er wird,
 ...
 * 
 * Annahme/Anmerkung:  
 * 			  (ganz viele 50 Werte f�r Rotanteil)
 * 			  Eindeutige Sortierung bringt keine Optimieren, da der Abstand immer mindestens
 * 			  1 betr�gt, d.h. man w�rde zwar einen guten Einstiegspunkt finden in allen
 * 			  50ern, aber m�sste ja trotzdem durch alle durchgehen, da man sich mindestens
